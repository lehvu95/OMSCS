...
Please also make sure to read the whole assignment before getting started and to follow the instructions we provide to the letter (e.g., **use the exact (to the letter) commit messages provided in the assignment, rather than variations of them**). If you don’t know how to accomplish a task, either consult Git’s help by running “`git --help <command>`” or leverage online resources (there are plenty, such as this [Git cheat sheet](https://drive.google.com/file/d/0B6urvdQprh5NeTNJcm9sNGxqNDQ/view?usp=sharing&resourcekey=0-7CHcB-s0h6SO5WfbsnM0Sw) or StackOverflow’s [Git for beginners](https://stackoverflow.com/questions/315911/git-for-beginners-the-definitive-practical-guide)​​). If you receive an error while executing a Git command, make sure to read the error message–Git often suggests exactly the right thing to do. Every time you ask the teaching staff to do something that has been clearly stated in this assignment that we won’t do for you, your points may be deducted.
...
3. Clone REPO
4. This should create a directory called `6300Spring26<your GT username>` under directory `User1`.
5. Go to directory `6300Spring26<your GT username>` (here you can also open the Network view in GT GitHub and start monitoring how your repository evolves)
6. Make sure that the directory contains a file called `MyPrivateRepo`
7. Create and go to directory `Assignment2` (under `6300Spring26<your GT username>`)
...
3. Clone REPO
4. Just like before, this should create a directory called `6300Spring26<your GT username>` under directory `User2`
5. Go to directory `6300Spring26<your GT username>/Assignment2`
...
15. In the main branch, edit file `myinfo.txt `and add the semester and year (i.e., “Spring 2026”) in a separate line.
...
3. Edit file `myinfo.txt `and add this text on the next single line of the file, using your own username: “`6300Spring26<your GT username>`”
4. Commit the file to your local repo with comment “Edited myinfo file for the third time”
5. Push the main branch (not the other branches) to the remote repository. **To do so, you will have to pull changes (using merge strategy, i.e. do NOT rebase) from the remote repository and suitably handle conflicts**. In handling conflicts, make sure not to lose any content, not to have any of the extra text added by Git to mark the conflicting parts, and to preserve the order of the information as it appears in the assignment on **four separate lines: first and last name, GT email, semester, and repository name (i.e., 6300Spring26<your GT username>)**. Use commit message “Final merge with conflicts fixed” for the additional commit after merging and handling the conflicts.
6. Tag the current version of the main as “V1” and push the tag to the remote repository. Use a [LIGHTWEIGHT tag](https://git-scm.com/book/en/v2/Git-Basics-Tagging).
...
- Run in each of the two terminals (one for User1 directory and the other one for User2 directory), and from the root of your repo (directory `6300Spring26<your GT username>`)
  - `git checkout main`
  ...
  - `git push --force`
- After resetting the state, there is no need to re-clone the repository (i.e., there is no need to perform the first 5 steps of the [instructions](#assignment-instructions)).

## Repo Reset if you lost the initial commit:

- If you somehow destroyed the “Initial commit” do the following.
  - Revert to the earliest commit as instructed above "Repo reset instructions". (In this case it won't be an "initial commit" -- but do it anyway).
  - Do 'git rm' to any files that aren't ".gitignore" or "MyPrivateRepo ("MyPrivateRepo" should be an empty file.
  - Be sure to add these files (git add <file name(s)>)
- Now that you've reverted to the earliest commit and re-established all the data as it should be. You'll just alter the commit and its message with:
