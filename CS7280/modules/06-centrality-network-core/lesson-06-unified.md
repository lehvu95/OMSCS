...
- Sabrin, K., Dovrolis, C. [The Hourglass Effect in Hierarchical Dependency Networks](https://www.cambridge.org/core/journals/network-science/article/hourglass-effect-in-hierarchical-dependency-networks/DDBCA83D16CA74B827DAB66A98CC906A), Journal of Network Science, (2017) — see [[hourglass-effect-hierarchical-dependency-networks.pdf|local copy]]
- Faskowitz, J., Yan, X., Zuo, X. *et al.* [Weighted Stochastic Block Models of the Human Connectome across the Life Span](https://www.nature.com/articles/s41598-018-31202-1). *Sci Rep* **8**, 12997 (2018) — see [[weighted-stochastic-block-models-connectome.pdf|local copy]]
...
### Video: Introduction to Centrality and Network-core Metrics and Algorithms

[[sections/00-lesson-overview.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_n5ob0mg6

| Text |
|------|
| In many network science applications, we're given a network and we are asked to find the |
| most important or central nodes or raises according to certain criteria. For example, we |
| may be given a communication network formed by all interactions between a group of |
| terrorists. And we asked to identify the leader of the group, or the group member that |
| probably knows the most information about the group's actions. Different importance |
| criteria lead to different centrality metrics, such as closeness centrality or |
| betweenness centrality. The objective of this lesson is to introduce you to the most |
| common centrality metrics in the literature and explain what notion of node importance |
| each of them tries to capture. A related problem is to identify the most important group |
| of nodes in a network. For example, in the case of the terrorist network, the objective |
| may be to identify the inner circle of the group. This relates to different mathematical |
| notions of core periphery structure, and we will review the most commonly used sites |
| approaches, such as the cake or the composition method, or the reads club notion. We will |
| also see a couple of applications of this matrix, including an analysis of all character |
| interactions in the Game of Thrones sequence of novels. |

## 01 — Degree, Eigenvector and The Katz Centrality

### Degree and Strength Centrality

![[img/01-degree-centrality.png|500]]
...
![[img/01-eigenvector-centrality.png|500]]
...
where $v$ is the vector of all node centralities. This equation, however, is simply the *eigenvector* definition of matrix $A$: $\lambda$ is the corresponding eigenvalue for eigenvector $v$. This is why we refer to this centrality metric as *"eigenvector centrality"*.
...
![[img/01-katz-centrality.png|500]]
...
## 02 — PageRank Centrality

![[img/02-pagerank-centrality.png|500]]
...
In fact, PageRank is only a slight modification of the Katz centrality metric. It is based on the following idea: if a node $j$ points to a node $i$ (and thus $A_{i,j} = 1$), and node $j$ has $k_{j,out}$ outgoing connections, then the centrality of node $j$ should be "split" among those $k_{j,out}$ neighbors. In other words, the "wealth" of node $j$ should not be just inherited by all nodes it points to – but rather, the "wealth" of node $j$ should be split among those nodes.

So, the defining equation of PageRank centrality becomes:

$$v_i = \beta + \frac{1}{\lambda} \sum_j A_{i,j}\, \frac{v_j}{k_{j,out}}$$

where the summation is over all nodes $j$ that point to $i$ (and thus, $k_{j,out}$ is non-zero).
...
$$v = \left(I - \frac{1}{\lambda} A\, D\right)^{-1} \cdot \mathbf{1}$$

where $D$ is a diagonal $n$-by-$n$ matrix in which the $j$'th element is $1/k_{j,out}$ if $k_{j,out}$ is non-zero (the diagonal elements for which $k_{j,out} = 0$ simply do not matter).
...
In practice, the computation of both Katz and PageRank centralities is performed numerically, using a power-iteration method that iterates the computation of the centrality values until those values converge. Typical values for $\frac{1}{\lambda}$ and $\beta$ are $0.85$ and $\left(1 - \frac{1}{\lambda}\right)/n$, respectively – but as in the case of Katz centrality, it is theoretically possible that the PageRank centrality computation does not converge if $\lambda$ is too low.
...
![[img/03-closeness-centrality.png|500]]
...
![[img/03-harmonic-centrality.png|500]]

An improved metric is harmonic centrality, defined as:

$$v_i = \sum_{j \neq i} \frac{1}{d_{i,j}}$$

Here, if nodes $i$ and $j$ cannot reach each other, the corresponding distance can be thought of as infinite, and thus the term $\frac{1}{d_{i,j}}$ is 0. Sometimes, the harmonic centrality is normalized by $\frac{1}{n-1}$ – but that does not affect the relative ordering of node centralities.
...
## 04 — Betweenness Centrality Variants

### Shortest-path Betweenness Centrality

![[img/04-shortest-path-betweenness.png|500]]
...
![[img/04-shortest-path-edge-betweenness.png|500]]
...
![[img/05-path-centrality-complexity-generality.jpg|600]]
...
## 06 — The Notion of "Node Importance"
...
![[img/07-k-core-spreaders.png|500]]
...
A simple algorithm, known as *"k-core decomposition"*, can associate each node with the highest *k-core* order that that node belongs to. The algorithm proceeds iteratively by removing all nodes of degree $k$, for $k = 0, 1, 2, \ldots$ During the iteration that the algorithm removes nodes of degree $k$, some higher-degree nodes may lose connections and become of degree-$k$. Those nodes would also be removed, joining the *k-core* until no further nodes can be removed in that iteration. The algorithm terminates when all nodes have been removed. Note that the maximum-order core set may not form a connected component.
...
## 08 — Core-Periphery Structure

### Video: Core-periphery Structure

[[sections/08-core-periphery-structure.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_ya47cr27

| Text |
|------|
| Another related concept is that some networks have a core-periphery structure. |
| Intuitively this means that the nodes of such networks can be partitioned in two groups, |
| the core nodes and the periphery nodes. The core nodes are very densely connected to each |
| other, and the rest of the network. The periphery notes on the other hand, are well |
| connected only to core nodes, not to other periphery nodes. The visualization here |
| contrasts it's synthetically generated core periphery network, with a randomly generated |
| network of the same number of nodes and. The core nodes appear in red, and they are about |
| 25% of all nodes. The block model matrix at the right, shows the probability that the |
| core node connects to another core node is 75%. The probability that a core node connects |
| to the periphery node is 33%. And the probability that two periphery nodes connect to |
| each other is only 10%. There are several algorithms in the literature that try to detect |
| first, whether a network has a core periphery structure, and if so, to identify the set |
| of core nodes. It is important to do so for an arbitrary number of nodes in the core set, |
| and to establish the statistical significance of the detected core, using an appropriate |
| null model. Meaning an ensemble of random networks that preserve the degree distribution, |
| and potentially other characteristics of the original network. We will review a specific |
| approach to detect if a network has a core periphery structure, in the next page. |

## 09 — Rich-Club Set of Nodes
...
![[img/09-rich-club-effect.jpg|600]]
...
## 10 — Core Set of Nodes in DAGs

![[img/10-tau-core-dag.jpg|600]]
...
The $\tau$-core is defined as follows: *given a set of network paths $P$ we are interested in, what is the minimal set of nodes that can cover at least a fraction $\tau$ of all paths in $P$?*
...
The problem of computing the *"$\tau$-core"* has been shown to be NP-Complete in the following paper. It can be approximated by a greedy heuristic that iteratively selects the node with the maximum number of remaining un-covered paths until the $\tau$ constraint is met.
...
[[sections/11-applications.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_stfg3mka
...
See [[lesson06-knowledge-check]] for the ungraded knowledge check questions.
