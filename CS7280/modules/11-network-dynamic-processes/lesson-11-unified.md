...
- "Synchronization in complex networks" by A. Arenas et al., 2008 — see [[synchronization-in-complex-networks.pdf|local copy]].
- "Adaptive coevolutionary networks: a review" by T. Gross and B. Blasius, 2008 — see [[adaptive-coevolutionary-networks-review.pdf|local copy]].
- "Co-evolutionary dynamics in social networks: A case study of twitter" by D. Antoniades and C. Dovrolis, 2015 — see [[coevolutionary-dynamics-social-networks.pdf|local copy]].
...
### Video: Introduction to Other Network Dynamic Processes

[[sections/00-lesson-overview.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_4iuqe4gu

| Text |
|------|
| In the last two lessons, we focused on spreading phenomena on networks, such as epidemics  |
| or information cascades. In this lesson, we will discuss some more dynamic network  |
| phenomena. Recall that dynamics of networks refers to problems in ways that topology  |
| changes over time, while dynamics on networks refers to time dependent processes that  |
| take place on a static network. We start with the problem of network robustness. What  |
| happens when we remove nodes from a network randomly, and what happens when we target  |
| high degree nodes? Then, we will talk about the problem of distributed circuits on  |
| networks. How do you find the node or some information on a network, when there is no  |
| central directory? The third problem we will discuss is, synchronization on networks. How  |
| is it possible that the large number of nodes, can get synchronized in a self organized  |
| manner, depending on the underlying network. The fourth problem relates to network  |
| control. How can we drive the state of each node in the network, to a desired operating  |
| point? And how can we do that by driving the minimum number of inputs? Finally, we close  |
| this lesson with coevolutionary or adaptive networks. These are networks in which the  |
| topology changes depending on the state of the nodes, but also the state of the nodes  |
| changes as a function of a topology. This feedback loop creates very interesting self  |
| organization phenomena. |

## 01 — Inverse Percolation On Grids and G(n,p) Networks
...
![[img/01-inverse-percolation-grid.jpg|400]]
...
![[img/01-inverse-percolation-gnp-failures-attacks.jpg|400]]

The visualization at the right shows simulation results of a $G(n,p)$ network with $n=10{,}000$ nodes and average degree $\bar{k}=3$.
...
## 02 — Random Removals (Failures) vs Targeted Removals (Attacks)

### Video: Random Removal vs Targeted Removal

[[sections/02-random-removals-vs-targeted-removals.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_y2xuzc8t
...
To derive the critical threshold of an inverse percolation process, we need a mathematical condition for the emergence of the giant component in a random network (including power-law networks). This condition is referred to as the **Molloy-Reed criterion** (or principle):
...
![[img/03-giant-component-illustration.png|450]]
...
Let $P[k_j=k \mid A_{i,j}=1]$ be the probability that node $j$ has degree $k$, given that nodes $i$ and $j$ are connected.

From Bayes's theorem, we have that:

$$P[k_j=k \mid A_{i,j}=1] = \frac{P[k_j=k, A_{i,j}=1]}{P[A_{i,j}=1]} = \frac{P[k_j=k] \, P[A_{i,j}=1 \mid k_j=k]}{P[A_{i,j}=1]} \quad (1)$$
...
Also, $P[A_{i,j}=1 \mid k_j=k] = \frac{k}{n-1}$ because $j$ chooses between $n-1$ nodes to connect to with $k$ edges (recall that the configuration model allows for multiple edges between nodes).

Returning to Equation (1), we can now derive the average degree of nodes in the LCC:

$$\sum_{k} k \, P[k_j=k \mid A_{i,j}=1] = \sum_{k} k \, \frac{P[k_j=k] \, P[A_{i,j}=1 \mid k_j=k]}{P[A_{i,j}=1]} = \frac{\sum_{k} k^2 p(k)}{\bar{k}} = \frac{\bar{k^2}}{\bar{k}}$$
...
## 04 — Robustness of Networks to Random Failures
...
![[img/04-critical-threshold-failures.png|600]]
...
## 05 — Robustness of Networks to Attacks
...
![[img/05-failures-vs-attacks-critical-threshold.png|600]]
...
## 06 — Small-world Networks and Decentralized Search

### Video: Small-world Networks and Decentralized Search

[[sections/06-small-world-networks-and-decentralized-search.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_xpgyc1s8

| Text |
|------|
| Back in the late 60s, the famous sociologist Stanley Milgram, decided to study the small  |
| world phenomenon. Until then, this was only a fascinating anecdote. People would find it  |
| amusing every time it was discovered that two random individuals know the same person.  |
| Nobody had studied this empirically however, and the six degrees of separation principle  |
| was just an expression. Milgram asked several individuals in Nebraska to forward the  |
| letter to a target person in Boston. He gave participants the name, address and  |
| occupation of the target. But the participants could not just send the letter directly to  |
| the target. They had to forward the letter to a person they knew on a first name basis,  |
| with a goal of reaching the target as soon as possible. Most participants chosen  |
| acquaintance based on geographical and occupational information. It is reasonable to  |
| expect that these letters would never reach their destination. Because finding such a  |
| path between two random individuals, one in Nebraska and another in Boston would require  |
| a very long number of intermediates. Besides, it could be that even if there are short  |
| paths between these two individuals, their immediate people would not know that short  |
| paths exist. And so the letters would keep getting forwarded in the vicinity of the  |
| target but never actually reaching the target. Surprisingly, one third of the letters  |
| made it to the target. The histogram that you see here from Milgram original paper, shows  |
| that the median distance was six steps, just as the six degrees of separation principle  |
| predicted. What can we learn from Stanley Milgram's small world experiment? What does it  |
| reveal about the structure of the underlying social network? And what does it say about  |
| the efficiency of a distributed search process in small-world networks? These are  |
| questions that we answered in the next few pages. Using an elegant mathematical model  |
| that was developed by John Kleinberg at Cornell University. |

**Food For Thought**

It is interesting to read the original paper by Milgram (1967) — see [[milgram-small-world-problem.pdf|local copy]].

## 07 — Decentralized Search Problem
...
![[img/07-kleinberg-grid-model.png|450]]
...
## 08 — The Optimal Search Exponent in Two Dimensions
...
![[img/08-delivery-time-vs-exponent-q.png|450]]
...
## 09 — Why The Optimal Value of q is Equal to Two in a Two-Dimensional World?
...
![[img/09-distance-resolution-scales.png|400]]
...
The proof of the previous result, at least for $k=1$ and $k=2$, is clearly written in sections 20.7 and 20.8 of the Easley/Kleinberg textbook. I recommend you go through it.

## 10 — Decentralized Network Search in Practice

> Image Source: Textbook *"Networks, Crowds and Markets"* by Easley and Kleinberg. Figures 20.9 and 20.10.
...
![[img/10-rank-based-links.png|450]]
...
![[img/10-friendship-probability-vs-rank.png|450]]
...
## 11 — Synchronization of Coupled Network Oscillators

### Video: Synchronization of Coupled Network Oscillators

[[sections/11-synchronization-of-coupled-network-oscillators.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_y6leujtc

| Text |
|------|
| Synchronization is a very important property of both natural and technological systems.  |
| In broad terms, synchronization refers to any phenomenon in which several distinct but  |
| coupled, dynamic elements have a coordinated dynamic activity. For example, all the  |
| devices that are connected to the Internet have synchronized clocks using a distributed  |
| protocol called NTP, Network Time Protocol. Interestingly, this protocol works quite well  |
| despite the fact that, Internet delays are unknown and time varying and the complete  |
| topology of the internet is not known. A classic example of emergent synchronization is  |
| shown in this YouTube video. Five metronomes are placed on a moving bar so that the  |
| motion of its metronome is loosely coupled with the motion of others. The metronomes  |
| start from a random phase and under certain conditions that we will discuss later in this  |
| lesson, they can get completely synchronized after some time as this video shows. Note  |
| that in this case, the communication between the five dynamic elements is only indirect.  |
| The metronomes do not exchange any messages instead, each of them affects to a small  |
| degree, the movement of the underlying board and that board affects the movement of every  |
| metronome. Another example of self organized synchronization in nature is when many  |
| fireflies gather close together as this image shows. Note that its firefly cannot see all  |
| other fireflies such global communication is not necessary in synchronized systems.  |
| Instead, the remarkable effect is that a large number of systems can get synchronized at  |
| least for a period of time, even if each of them can only communicate locally with other  |
| nearby systems. Similar distributed synchronization phenomena also take place with flocks  |
| of birds, and schools of fish, forming fascinating dynamic patterns. In technology,  |
| similar problems emerge when we have a group of robots, autonomous vehicles or drones  |
| that need to coordinate without having a centralized controller. Our brains also rely on  |
| short term synchronization between thousands of neurons, so that different brain regions  |
| can communicate. This type of synchronization, referred to as coherence is shown at this  |
| video. The video shows three views from the lateral and dorsal of the zebrafish brain  |
| using calcium imaging. Whenever a cluster of neurons fires, the corresponding part of the  |
| brain lights up. Note how different brain regions get active at about the same time,  |
| occasionally producing large avalanches of neural activity throughout most of the brain.  |
| Synchronization is not always a desirable state, however, for example, too much  |
| synchronization in the brain can cause seizures. This image shows EEG recordings during  |
| the onset of an epileptic seizure. The scissor starts at about the middle of the blood,  |
| and it shows major waves discharges at the frequency of about three hertz over most of  |
| the patient's cortical surface. |

**Food For Thought**

Can you think of other systems, closer to your interests, that can show spontaneous synchronization?

## 12 — Coupled Phase Oscillators – Kuramoto Model
...
![[img/12-kuramoto-synchronization-simulation.png|600]]
...
![[img/12-order-parameter-complex-plane.png|400]]
...
## 13 — Kuramoto Model on a Complete Network
...
![[img/13-order-parameter-vs-coupling-strength.png|450]]
...
## 14 — Kuramoto Model on Complex Networks
...
![[img/14-synchronization-power-law-network.png|450]]
...
## 15 — Adaptive (or Coevolutionary) Networks
...
![[img/15-adaptive-network-feedback-loop.png|400]]
...
## 16 — Consensus Formation in Adaptive Networks
...
$$o(j, t + 1) = o(j, t) - \mu \left(o(j, t) - o(i, t)\right)$$

where $\mu \in [0, 1/2]$ is a convergence parameter. Consider the case of $\mu = \frac{1}{2}$ that corresponds to $i$ and $j$ adopting the same intermediate opinion after communication.

The model considers two coexisting dynamical processes: local opinion convergence for agents whose opinions are within the tolerance range, and a rewiring process for agents whose opinions differ more. The relative frequencies of these two processes is quantified by the parameter $w \in [0, 1]$.
...
![[img/16-consensus-formation-adaptive-vs-static.png|600]]
...
Figure (B) refers to the case of a static network without any rewiring, while Figure (C) refers to the adaptive model we described above (with $w=0.7$). The evolution of the opinion of a few individuals is highlighted with color.
...
## 17 — Coevolutionary Effects in Twitter Retweets
...
![[img/17-speaker-repeater-listener.png|450]]
