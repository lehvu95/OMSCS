...
### Video: Introduction to Modularity and Community Detection

[[sections/00-lesson-overview.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_nspohts0
...
## 01 — The Graph Partitioning Problem

![[img/01-minimum-bisection.png|400]]
...
## 02 — Network Community Detection

### Video: Community Detection Problem

[[sections/02-community-detection-problem.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_rnycjiyx
...
### Video: Modular Networks: The Adjacency Matrix View

[[sections/02-modular-networks-adjacency-matrix-view.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_cjbirgdr
...
### Video: Example Network Communities: Zachary's Karate Club

[[sections/02-zacharys-karate-club.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_ur18rgh0
...
## 03 — Community Detection Based on Edge Centrality Metrics

![[img/03-edge-betweenness-girvan-newman.jpg|500]]
...
## 05 — Agglomerative Hierarchical Clustering Approaches - Node Similarity Metric

Let us now switch to agglomerative *(or bottom-up)* hierarchical clustering algorithms.

![[img/05-ravasz-algorithm.jpg|600]]
...
## 06 — Hierarchical Clustering Approaches – Group Similarity

![[img/06-single-complete-average-similarity.jpg|600]]

*Image 9.10 from networksciencebook.com. Three approaches, called single, complete and average cluster similarity, are frequently used to calculate the community similarity from the node-similarity matrix $x_{ij}$.*
...
![[img/06-hierarchical-tree-cut.jpg|600]]
...
## 07 — Modularity Metric

![[img/07-given-vs-randomized-network.jpg|600]]
...
Additionally, we are given partitioning of all nodes to a set of $C$ hypothetical communities $C_1, C_2, \ldots C_C$.
...
## 08 — Modularity Metric- Derivation

![[img/08-modularity-derivation.jpg|600]]
...
$$\kappa_c^2 = \left(\sum_{i \in C_c} k_i\right)\left(\sum_{j \in C_c} k_j\right) = \sum_{(i,j) \in C_c} (k_i k_j)$$
...
## 09 — Modularity Metric – Selecting The Community Assignment

![[img/09-partition-quality-examples.jpg|600]]
...
## 10 — Modularity After Merging Two Communities

![[img/10-merging-communities.png|600]]
...
## 11 — Greedy Modularity Maximization

### Video: Greedy Modularity Maximization

[[sections/11-greedy-modularity-maximization.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_hcjlofqo
...
![[img/12-louvain-algorithm.png|600]]
...
$$\Delta M_{i,A} = \left(\frac{W_{A,int} + 2\, W_{i,A}}{2\, W} - \left(\frac{W_A + W_i}{2\, W}\right)^2\right) - \left(\frac{W_{A,int}}{2\, W} - \left(\frac{W_A}{2\, W}\right)^2 - \left(\frac{W_i}{2\, W}\right)^2\right)$$

where $W_A$ is the sum of weights of all links in $A$, $W_{A,int}$ is the sum of weights of all internal links in $A$, $W_i$ is the sum of weights of all links of node $i$, $W_{i,A}$ is the sum of weights of all links between node $i$ and any node in $A$, and $W$ is the sum of weights of all links in the network.
...
![[img/13-resolution-merged-cliques.jpg|500]]
...
$$\Delta M_{AB} = \frac{1}{L} - \frac{\kappa_l^2}{2 L^2} = \frac{1}{L}\left(1 - \left(\frac{\kappa_l}{2L}\right)^2\right)$$

So, the merging of the two small communities will take place if $\Delta M_{AB} > 0$ which is equivalent to $\kappa_l < \sqrt{2 L}$.
...
## 14 — The Modularity Search Landscape

> Note that there is an error in the video: (0:37) *"Subsequent two assignments have even larger modularity."* It should be: *"Subsequent assignment b have even larger modularity."*

### Video: The Modularity Search Landscape

[[sections/14-modularity-search-landscape.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_0mxjiqci
...
## 15 — Communities Within Communities

![[img/15-communities-within-communities.jpg|600]]
...
## 16 — Clustering Coefficient Versus Degree in Practice

![[img/16-clustering-coefficient-vs-degree.jpg|600]]
...
## 17 — Hierarchical Modularity Through Recursive Community Detection

![[img/17-greedy-algorithm-dendrogram.jpg|600]]
...
See [[lesson07-knowledge-check]] for the ungraded knowledge check questions.
