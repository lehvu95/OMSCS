# Lesson 5 — Network Paths, Clustering and The "Small World" Property

> Source: Canvas — CS-7280-O01, OAN (Summer 2026)

## 00 — Lesson Five Overview
...
- Sections 3.8, 3.9, 5.10 from A-L. Barabási, [Network Science](http://www.networksciencebook.com/), 2015
...
- [Structure and function of the feed-forward loop network motif](https://doi.org/10.1073/pnas.2133841100), S. Mangan and U. Alon, PNAS October 14, 2003 100 (21) 11980-11985 — see [[mangan-alon-2003-feed-forward-loop-network-motif.pdf|local copy]]
...
- Lesson-5: Quiz
- Module Two [Assignment]: Project Two

### Video: Introduction to Network Paths, Clustering and The "Small World" Property

[[sections/00-lesson-overview.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_3tb87i1j

| Text |
|------|
| The objective of this lesson is to introduce you to the very important concept of small |
| world networks. Perhaps you already use this term whenever you find out that you have a |
| social connection through one or two other people with someone you met randomly. As we |
| will see in this lesson, small world networks are everywhere. Not only in the context of |
| social relationships. To understand the concept of small world networks, we will need to |
| start from two other ideas. First, the notion of network clustering, which relates to how |
| often we see triangles of connected nodes and second, the notion of network efficiency. |
| Which relates to how close pairs of nodes are to each other. We will also compare small |
| world networks with random, regular, and parallel networks. We will close this lesson |
| with a generalization of triangles that is referred to as network motifs. Motifs can |
| often be thought of as important functional modules in a network. |
...
![[img/01-clustering-coefficient-examples.jpg|600]]
...
![[img/01-clustering-coefficient-ck-plot.jpg|360]]
...
## 02 — Average Clustering and Transitivity Coefficient
...
![[img/02-average-clustering-vs-transitivity.jpg|600]]
...
## 03 — Clustering in Weighted Networks
...
![[img/03-weighted-clustering.jpg|500]]
...
## 04 — Clustering in G(n,p) networks
...
![[img/real-world-clustering-vs-size.jpg|360]]
...
## 05 — Clustering in Regular Networks
...
![[img/05-regular-ring-lattice.jpg|400]]
...
## 06 — Diameter, Characteristic Path Length, and Network Efficiency
...
![[img/06-yeast-ppi-path-distribution.jpg|400]]
...
![[img/06-facebook-path-distribution.jpg|400]]
...
## 07 — Diameter and CPL of G(n,p) Networks

Let us now go back to the simplest random graph model we have studied so far, i.e., the ER or $G(n,p)$ model. What is the diameter of such a network? We can derive an approximate expression as follows.
...
![[img/07-diameter-cpl-gnp.jpg|500]]
...
Specifically, for very sparse networks, as $\bar{k}$ approaches to 1 from above (so that the network is still expected to have a large connected component), the diameter is expected to be
...
## 08 — Diameter and Efficiency of G(n,p) Versus Regular Networks
...
Similarly, in three dimensions each node has 6 neighbors, and the diameter grows as $O(n^{1/3})$ – and so on in higher dimensions.
...
![[img/08-gnp-vs-regular-diameter.jpg|700]]

Let us go back to the hypothetical social network of 7 billion humans – and with an average of 6 connections per person. If this social network was a regular lattice, it would need to have 3 dimensions so that each node has 6 connections. So the diameter would be in the order of $n^{1/3}$, which is about 1,913 – this is much larger than the diameter we derived earlier for a random network of the same size and density - this being about 22.
...
## 09 — What Does "Small-world" Network Mean?
...
The three videos below form a 3-part series covering the empirical observation that motivates the small-world property, the Watts-Strogatz model that produces it, and the degree distribution of that model.

### Video: Clustering and Efficiency in Real-world Networks

[[sections/09-clustering-and-efficiency-in-real-world-networks.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_1r2fgmek

| Text |
|------|
| So far this lesson has focused on two network properties. The presence of clustering, and |
| the relation between diameter and network size. You may wonder what about real-world |
| networks? Where do they stand in terms of these two properties? An important discovery by |
| Watts and Strogatz in 1998 was that the networks we often see in practice have the |
| following two properties, they have strong clustering similar to that of lattice networks |
| with the same average degree, and second, they have short paths similar to the |
| characteristic path length and diameter we see in random Erdos-Renyi networks that have |
| the same size n, and density P as the real-world network we were given. We refer to such |
| networks as small-world networks. They are small in the sense that the shortest paths |
| between nodes increase only modestly with a size of the network. Modestly means |
| logarithmically or even slower. At the same time, small-world networks are not randomly |
| formed. On the contrary, the nodes form clusters of interconnected triangles, similar to |
| what we see in social groups, such as families, groups of friends, or larger |
| organizations of people. The table that you see here shows the characteristics of some |
| real-world networks. The columns show the network name, the number of nodes n, the number |
| of edges l, the average degree, the characteristic path length, the diameter, and the |
| predicted diameter based on the formula we derived earlier in the last column. Note that |
| the characteristic path length in the same order of magnitude with what we would expect |
| from a GNP random network. Additionally, this plot shows the clustering coefficient for |
| each of these networks with different colors. All networks have a much larger clustering |
| coefficient than what would be expected from a corresponding GNP network. |

### Video: Watts Strogatz Model

[[sections/09-watts-strogatz-model.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_p3bpfu4x
...
### Video: Degree Distribution of Watts Strogatz Model

[[sections/09-degree-distribution-of-watts-strogatz-model.mp4]]

Video URL: https://mediaspace.gatech.edu/media/t/1_gf2itfdg

| Text |
|------|
| In Lesson 4, we focused on the degree distribution of real-world networks, and so that |
| many such networks have a power load degree distribution. You may wonder, is the |
| Watts-Strogatz Model able to produce networks with a power load degree distribution? The |
| answer is no. The degree distribution of that model depends on the rewiring probability |
| p. If p is close to 0, most nodes have the same degree. As p approaches 1, we get the |
| Poisson degree distribution of a random graph. In every way, the resulting degree |
| distribution is not sufficiently skewed towards higher values. It cannot be |
| mathematically modeled as a power load and we do not see hubs. In summary, even though |
| the Watts-Strogatz Model was a great first step in discovering two important properties |
| of real-world networks, clustering and short paths, it is not a model that can construct |
| realistic networks because it does not capture the degree distribution of many real-world |
| networks. |
...
![[img/10-clustering-pa-vs-size.jpg|500]]
...
## 11 — Length of Shortest Paths in PA Model
...
![[img/11-shortest-paths-pa-model.jpg|600]]
...
## 12 — Path Lengths in Power-law Networks
...
![[img/12-path-lengths-power-law.jpg|600]]

The plots at the lower part of the visualization show shortest-path length distributions for three different exponent values as well as for a corresponding $G(N,p)$ random graph. As you see, the differences between all these networks are minor when the network is only a few hundreds of nodes. For networks with millions of nodes, however, we see a major difference in the resulting path length distributions, showing clearly the major effect of the degree distribution exponent as well as the critical value of $\gamma$.

**Food For Thought**

Look at the literature for a derivation of the previous formula for the CPL of networks when $\gamma$ is between 2 and 3.

## 13 — Directed Subgraph Connectivity Patterns
...
![[img/13-directed-subgraph-patterns.jpg|600]]

The 13 patterns are named: fan out, fan in, cascade, mutual out, mutual in, bi-mutual, FFL (Feed-Forward Loop), FBL (Feedback Loop), regulating mutual, regulated mutual, mutual cascade, semi clique, and clique.
...
## 14 — Statistical Test For The Frequency of a Network Motif

![[img/14-statistical-test-motif.jpg|600]]
...
## 15 — Frequent Motifs and Their Function
...
![[img/15-motifs-figure-1.jpg|600]]
...
See [[sections/16-knowledge-check.pdf]].
